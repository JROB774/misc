...
template<typename T> T& Array<T>::AddElement (const T& _element)
{
	...
	if (data) { return data[count++] = _element; }
	return data[count];
}
template<typename T> T& Array<T>::AddElementAtPosition (const T& _element, size_t _pos)
{
	// Do not bother if the position is out of bounds.
	if (_pos > count) { return data[count]; }
	...
	size_t count_after_pos = count - _pos;
	memmove(data+_pos+sizeof(T), data+_pos, count_after_pos * sizeof(char));
...
template<typename T> void Array<T>::RemoveElementAt (size_t _index)
{
	// Do not bother if the element to remove doesn't exist.
	if (_index >= count) { return; }
	// Do not bother if the array is empty.
	if (count == 0) { return; }

	// Move the contents after _index back by one.
	size_t count_after_index = count - _index;
	memmove(data+_index, data+_index+1, count_after_index * sizeof(T));

	// The array has now shrunk by one element.
	--count;
}
...
template<typename T> void Array<T>::Resize (size_t _size)
{
	...
	// constructor and destructor that must be handled to avoid issues (e.g. TEIN_String).
