...
// @TOOD:
// - fix the problem with sort crashing
// - fix the problem with crashing on image load
// - add offsets (content x & y) to the ref so that all images stay aligned!!!
...
struct Image
{
	...
	Image (Image&& _img) { *this = _img; _img.pixels = nullptr; _img.Destroy(); }
...
inline std::vector<Rect> PackTexture (std::vector<Image>& _images, int& _texturesize, int& _power)
{
	...
	for (/* NULL */; _texturesize < maximgside; ++_power) { _texturesize = std::pow(2, _power); }
	...
	if (_images.size() > 1) {
		...
		for (auto image = _images.begin()+1; image != _images.end(); ++image) {
			...
			while (badplace) {
				...
				if (bounds.x + bounds.w > _texturesize) {
...
int main (int _argc, char* _argv[])
{
	...
	std::sort(
		...
		{ return (_lhs.content.GetSize() > _rhs.content.GetSize()); }
	);
	...
	int texturesize = 0;
	int power = 0;
	auto texturedata = PackTexture(images, texturesize, power);
	...
	buffer.resize((texturesize * texturesize) * Image::BPP, 0);
	// stores the texture pitch (saves on performing the calculation repeatedly)
	int texturepitch = texturesize * Image::BPP;
	...
	for (unsigned int i = 0; i < images.size(); ++i) {
		...
		// writes the identifier and boundary data to the file (in plain-text ASCII)
		reference << refname << " " << texturedata.at(i) << "\n";
	}
